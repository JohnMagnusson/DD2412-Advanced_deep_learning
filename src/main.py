...
if do_warmup_new_model:
    ...
    trained_model.save_weights("warmedup_models/simCLR_model_weights_resnet50_warmedup")
...
if do_train_new_model:
    ...
    model = build_simCLR_model(encoder_network="resnet-50", projection_head_mode="nonlinear")
    model.load_weights("warmedup_models/simCLR_model_weights_resnet50_warmedup")
    trained_model, training_loss, validation_loss = train_model(model, train_data, val_data)
    trained_model.save_weights("saved_models/simCLR_model_weights_resnet50")
...
if do_fine_tune_model:
    ...
    model = build_simCLR_model(encoder_network="resnet-50", projection_head_mode="nonlinear")
    model.load_weights("saved_models/simCLR_model_weights_resnet50")
    ...
    fine_tuned_model.save_weights("finetuned_models/simCLR_model_weights_resnet50")
