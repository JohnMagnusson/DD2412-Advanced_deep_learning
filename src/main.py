...
if do_warmup_new_model:
    ...
    trained_model.save_weights("warmedup_models/simCLR_model_weights_2_warmedup")
    print("Done with warmup")

if do_train_new_model:
    print("Starting real training")
    model = build_simCLR_model(encoder_network="resnet-18", projection_head_mode="nonlinear")
    model.load_weights("warmedup_models/simCLR_model_weights_2_warmedup")
    ...
    plot_loss(training_loss, validation_loss)
    trained_model.save_weights("saved_models/simCLR_model_weights")
...
if do_fine_tune_model:
    ...
    fine_tuned_model.save_weights("finetuned_models/simCLR_model_fine_tuned")
