...
if do_warmup_new_model:
    ...
    trained_model.save_weights("warmedup_models/resnet_50_long")
...
if do_train_new_model:
    ...
    model = build_simCLR_model(encoder_network=encoder_network, projection_head_mode=projection_head)
    model.load_weights("warmedup_models/resnet_50_long")
    trained_model, training_loss, validation_loss = train_model(model, train_data, val_data)
    trained_model.save_weights("saved_models/resnet_50_long")
...
if do_fine_tune_model:
    ...
    model = build_simCLR_model(encoder_network=encoder_network, projection_head_mode=projection_head)
    try:    #todo REMOVE! Temporary for long run
        model.load_weights("checkpoint_models/resnet_50_long")
    except:
        print("Could not load model at: checkpoint_models/resnet_50_long")
        print("Will load saved model instead")
        model.load_weights("saved_models/resnet_50_long")
    ...
    fine_tuned_model.save_weights("finetuned_models/resnet_50_long")
    ...
    save_test_accuracy(test_acc, "resnet_50_fine_tuning")
...
if do_linear_evaluation:
    ...
    sk_learn_model, val_accuracy, test_acc = linear_evaluation_model(model, train_data, val_data, test_data,
                                                                     "nonlinear")
    pickle.dump(sk_learn_model, open("linear_evaluation/rand_augment_3_5", 'wb'))
    plot_linear_evaluation_accuracy(val_accuracy, should_save_figure=True, file_name="linear_evaluation")
    save_test_accuracy(test_acc, "resnet_50_linear_evaluation")
...
if do_evaluation_on_model:
    ...
    model = build_simCLR_model(encoder_network=encoder_network, projection_head_mode=projection_head)
    model.load_weights("finetuned_models/resnet-18_tets")
