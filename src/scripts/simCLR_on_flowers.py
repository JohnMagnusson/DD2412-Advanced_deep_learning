...
if do_warmup_new_model:
    ...
    model = build_simCLR_model(encoder_network=encoder_network, projection_head_mode="nonlinear", weight_decay=False)
    trained_model, training_loss, validation_loss = warmup_model(model, train_data, val_data)
    trained_model.save_weights("warmedup_models/resnet_18_cifar10-no_l2")
    plot_loss(training_loss, validation_loss, should_save_figure=True, file_name="resnet_18_cifar10-no_l2_warmup_loss")
    print("Done with warmup")

if do_train_new_model:
    print("Starting real training")
    model = build_simCLR_model(encoder_network=encoder_network, projection_head_mode="nonlinear", weight_decay=False)
    model.load_weights("warmedup_models/resnet_18_cifar10-no_l2")
    trained_model, training_loss, validation_loss = train_model(model, train_data, val_data)
    trained_model.save_weights("saved_models/resnet_18_cifar10-no_l2")
    plot_loss(training_loss, validation_loss, should_save_figure=True, file_name="resnet_18_cifar10-no_l2_training_loss")
    print("Done with training")

if do_fine_tune_model:
    print("Starting with fine-tuning")
    model = build_simCLR_model(encoder_network=encoder_network, projection_head_mode="nonlinear", weight_decay=False)
    model.load_weights("saved_models/resnet_18_cifar10-no_l2")
    ...
    fine_tuned_model.save_weights("finetuned_models/resnet_18_cifar10-no_l2")
    plot_fine_tuning(history_fine_tune, should_save_figure=True, file_name="resnet_18_cifar10-no_l2_fine_tuning")
...
if do_linear_evaluation:
    ...
    plot_linear_evaluation_accuracy(val_accuracy, should_save_figure=True, file_name="resnet_18_cifar10-no_l2_linear_evaluation")
    pickle.dump(sk_learn_model, open("linear_evaluation/resnet_18_cifar10-no_l2", 'wb'))
...
if do_evaluation_on_model:
    ...
    model.load_weights("finetuned_models/resnet_18_cifar10-no_l2")
