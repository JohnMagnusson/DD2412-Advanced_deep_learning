...
if do_warmup_new_model:
    ...
    trained_model.save_weights("warmedup_models/resnet_18_cifar10-l2")
...
if do_train_new_model:
    ...
    model = build_simCLR_model(encoder_network=encoder_network, projection_head_mode="nonlinear", weight_decay=True)
    model.load_weights("warmedup_models/resnet_18_cifar10-l2")
    trained_model, training_loss, validation_loss = train_model(model, train_data, val_data)
    trained_model.save_weights("saved_models/resnet_18_cifar10-l2")
...
if do_fine_tune_model:
    ...
    model = build_simCLR_model(encoder_network=encoder_network, projection_head_mode="nonlinear", weight_decay=True)
    model.load_weights("saved_models/resnet_18_cifar10-l2")
    ...
    fine_tuned_model.save_weights("finetuned_models/resnet_18_cifar10-l2")
...
if do_linear_evaluation:
    ...
    pickle.dump(sk_learn_model, open("linear_evaluation/resnet_18_cifar10-l2", 'wb'))
...
if do_evaluation_on_model:
    ...
    model.load_weights("finetuned_models/resnet_18_cifar10-l2")
